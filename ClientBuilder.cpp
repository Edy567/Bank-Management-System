...
ClientBuilder& ClientBuilder::setNume(const std::string& n) { nume = n; return *this; }
ClientBuilder& ClientBuilder::setPrenume(const std::string& p) { prenume = p; return *this; }
ClientBuilder& ClientBuilder::setCNP(const std::string& c) { CNP = c; return *this; }
ClientBuilder& ClientBuilder::setParola(const std::string& p) { parola = p; return *this; }
ClientBuilder& ClientBuilder::setVenit(double v) { venit = v; return *this; }
ClientBuilder& ClientBuilder::setScorCredit(int s) { scorCredit = s; return *this; }
...
ClientBuilder& ClientBuilder::makeVIP() {
    ...
    const std::vector<Card> cEur = { Card(10000.0, "Titular VIP", "12/30", "5555666677778888", Moneda("EUR", "Euro", 4.97)) };
    if(Cont* p = ContFactory::creareCont("PREMIUM", cEur, "RO02VIP_EUR", {})) 
        conturiDeAtasat.push_back(p);


    const std::vector<Card> cGbp = { Card(5000.0, "Titular VIP", "12/30", "9999000011112222", Moneda("GBP", "Lira", 5.80)) };
...
ClientBuilder& ClientBuilder::makeStudent() {
    ...
    const std::vector<Card> cStud = { Card(500.50, "Student Bursier", "09/28", "1234123412341234", Moneda("RON", "Leu", 1)) };
